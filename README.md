...
The SAM CLI installs dependencies defined in `TranmereWebPlayerSearch/package.json`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
$ sam local invoke TranmereWebPlayerSearch --event events/event.json
```
...
```bash
$ npm run build
$ sam deploy --stack-name PlayerSearch --capabilities CAPABILITY_IAM --s3-bucket tranmere-web-api
```
...
```bash
$ sam logs -n TranmereWebPlayerSearchFunction --stack-name PlayerSearch --tail
```
...
```bash
$ aws cloudformation delete-stack --stack-name PlayerSearch
```
